...
I've already written the code for method one:
...
;; Naural -> Hash
;; given a range of integers, return a hash of the
;; step number and its triangle number
(define (step-v-hash velocity steps)
  (for/hash ([step (in-range steps)]
             [v (in-range velocity -1000 -1)])
    (values step (sum-of-numbers v))))

(define (velo edge v)
  (cond [(< edge 0) v]
        [else (velo (- edge v) (add1 v))]))
