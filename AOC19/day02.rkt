...
(require "intcode.rkt")  ; my intcode computer module

; load in starting data
(define DATA "input2.txt")
(define GAP (vector->immutable-vector                                   ; make it immutable for safety
             (list->vector                                              ; make a vector
              (map string->number                                       ; turn data into list of numbers
                   (string-split (file->string DATA) ",")))))               
...
(define p (vector-copy GAP))

(vector-set! p 1 12)
(vector-set! p 2 2)

(printf "AOC Problem 2.1 = ~a\n"
        (vector-ref (run-intcode p) 0))
...
; for the computer's memory. When you run an Intcode program, make sure to start by
; initializing memory to the program's values. A position in memory is called an address (for example, the first value in memory is at "address 0").
; 
...
(define FINAL 19690720)

(define (try-this noun verb)
  (local [(define v (vector-copy GAP))]
    (vector-set! v 1 noun)
    (vector-set! v 2 verb)
    (vector-ref (run-intcode v) 0)))

(for* ([noun 100] [verb 100]
       #:when (equal? (try-this noun verb) FINAL))
